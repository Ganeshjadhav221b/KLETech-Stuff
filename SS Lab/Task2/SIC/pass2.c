    ...
    while(fgets(str,30,fp))
    {
        ...
        if(word_count==4)
        {
            ...
            //printf("%s\t%s\n",opcode,value);
                ...
                {
                    //printf("%s",value);
                    fprintf(fp3,"%s",value);        ///if opcode is found in optab, write the opcode value from optab to the objectcode file as first 1 byte
                    rewind(fp2);
                    while(!feof(fp2))
                    {
                        fgets(str2,30,fp2);
                        sscanf(str2,"%s %s",&addr,&label);      ///search operands in symtab
                    ...
                    if(flag1==1)                                ///if found write it as next 2 bytes in the objectcode file.
                    {
                        fprintf(fp3,"%s\n",addr);
                    }
                    else
                    {
                        if(flag2==1)
                        {
                            ...
                            if(flag1==1)
                            {
                                ...
                                sscanf(addr,"%X",&x_addr);
                                x_addr=x_addr+32768;           ///hex value of 8000 is 32768
                                //printf("%X",x_addr);
                                fprintf(fp3,"%X\n",x_addr);
                            }
                            else
                ...
                else        ///if opcode is not found in optab
                {
                    i_count=0;
                    if((strcmp(col2,"WORD")==0))
                    {
                        ///if opcode is WORD count the no. of characters in operand field.
                        for(i=0;col3[i]!='\0';i++)
                        {
                            i_count++;
                        }
                        for(i=i_count;i<6;i++)
                        {
                        /// if less than 6 append 0's.
                            fprintf(fp3,"0");
                        }
                        fprintf(fp3,"%s\n",col3);
                        ///write the objectcode to the file
                    }
                    else if((strcmp(col2,"RESW")==0)||(strcmp(col2,"RESB")==0))
                    {
                        ///no action if opcode is RESW or RESB
                        continue;
                    }
                    else if(strcmp(col2,"BYTE")==0)
                    {
                        if(col3[0]=='C')
                        {
                            ///if byte is of charcter type,
                            for(i=2;col3[i]!='\0';i++)
                            {
                                int e=col3[i];
                                ///get the ascii equivalent hex value of the characters
                                //printf("%X",e);
                                c++;
                                if(c>3)
                                    break;
