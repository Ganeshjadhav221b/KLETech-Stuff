...
int pass1()
{
    ...
    while(fscanf(fp1,"%s",a))
    {
        ...
        if(strcmp(a,"D")==0)
        {
            ...
            {
                fscanf(fp1,"%s",a);
                if((strcmp(a,"R")==0)||strcmp(a,"T")==0||strcmp(a,"M")==0){
                    break;
                }
                fprintf(fp2,"\t%s\t",a);
                fscanf(fp1,"%x",&d);
...
{
    printf("PASS 2\n");
    printf("Starting address given by the user is %X",a);
    FILE *fp1 = fopen("input.txt","r");
    FILE *fp2 = fopen("ESTAB.txt","r");
    FILE *fp3 = fopen("mem.txt","w");
    char b[50],str[50];
